...
Usage:
    ...
    python can_websocket_bridge.py --channel can0 --port 8080

    # With a virtual CAN interface for testing:
    sudo modprobe vcan
    sudo ip link add dev vcan0 type vcan
    sudo ip link set up vcan0
    python can_websocket_bridge.py --channel vcan0 --port 8080
...
async def handler(ws: websockets.WebSocketServerProtocol, bus: can.BusABC):
    clients.add(ws)
...
async def main(channel: str, bustype: str, bitrate: int, port: int, host: str):
    ...
    async with websockets.serve(lambda ws, _path=None: handler(ws, bus), host, port):
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.channel, args.bustype, args.bitrate, args.port, args.host))
